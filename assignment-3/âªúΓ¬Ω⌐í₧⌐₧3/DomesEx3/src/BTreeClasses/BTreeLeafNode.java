...
public class BTreeLeafNode<TKey extends Comparable<TKey>, TValue> extends BTreeNode<TKey> {
	...
	//public static BPlusTreeMetrics BP_LEAF_NODE_METRICS= new BPlusTreeMetrics();
	
	public int M;
	
	public BTreeLeafNode(int M) {
		super(M);
	...
	public int search(TKey key) {
		BTree.bpMetrics.incrementNodeAccessCount();
		for (int i = 0; i < this.getKeyCount(); ++i) {
			 BTree.bpMetrics.incrementComparisonsCount();
	...
	protected BTreeNode<TKey> split() {
		...
		BTreeLeafNode<TKey, TValue> newRNode = new BTreeLeafNode<TKey, TValue>(M);
